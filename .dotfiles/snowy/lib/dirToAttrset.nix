# Stolen shamelessly from Nyyadia
let
  ...
  in
    ...
    then builtins.substring 0 (strLen - suffixLen) str
    else str;

  # Check if a directory has a default.nix file (with error handling)
  hasDefaultNix = dirPath: let
  ...
  dirToAttrset = dir: let
    ...
    processEntry = name: type:
      ...
      else if type == "directory" && hasDefaultNix (dir + "/${name}")
      then
